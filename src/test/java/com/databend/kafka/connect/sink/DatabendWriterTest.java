...
public class DatabendWriterTest {
    ...
    private DatabendWriter newWriter(Map<String, String> props) {
        final DatabendSinkConfig config = new DatabendSinkConfig(props);
        dialect = DatabendClient.create(config);
        final DbStructure dbStructure = new DbStructure(dialect);
        return new DatabendWriter(config, dialect, dbStructure);
    }

    private DatabendWriter newWriterWithMockConnection(Map<String, String> props, Connection mockConnection) {
        final DatabendSinkConfig config = new DatabendSinkConfig(props);
        dialect = mock(DatabendConnection.class);
    ...
    private SQLException verifyConnectionRollback(boolean succeedOnRollBack) throws SQLException {
        ...
        DatabendWriter writer = newWriterWithMockConnection(props, mockConnection);
    ...
    public void autoCreateWithAutoEvolve() throws SQLException {
        String topic = "books";
        TableIdentity tableId = new TableIdentity(null, null, topic);
        ...
        TableDefinition refreshedMetadata = dialect.describeTable(databendHelper.connection, tableId);
    ...
    public void multiInsertWithKafkaPkFailsDueToUniqueConstraint() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.INSERT, DatabendSinkConfig.PrimaryKeyMode.KAFKA, "");
    }

    @Test
    public void idempotentUpsertWithKafkaPk() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.UPSERT, DatabendSinkConfig.PrimaryKeyMode.KAFKA, "");
    }

    @Test(expected = SQLException.class)
    public void multiInsertWithRecordKeyPkFailsDueToUniqueConstraint() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.INSERT, DatabendSinkConfig.PrimaryKeyMode.RECORD_KEY, "");
    }

    @Test
    public void idempotentUpsertWithRecordKeyPk() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.UPSERT, DatabendSinkConfig.PrimaryKeyMode.RECORD_KEY, "");
    }

    @Test(expected = SQLException.class)
    public void multiInsertWithRecordValuePkFailsDueToUniqueConstraint() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.INSERT, DatabendSinkConfig.PrimaryKeyMode.RECORD_VALUE, "author,title");
    }

    @Test
    public void idempotentUpsertWithRecordValuePk() throws SQLException {
        writeSameRecordTwiceExpectingSingleUpdate(DatabendSinkConfig.InsertMode.UPSERT, DatabendSinkConfig.PrimaryKeyMode.RECORD_VALUE, "author,title");
    }
    ...
    private void writeSameRecordTwiceExpectingSingleUpdate(
        ...
        assertEquals(
                1,
                databendHelper.select("select count(*) from books", new DatabendHelper.ResultSetReadCallback() {
                    @Override
    ...
    public void sameRecordNTimes() throws SQLException {
        ...
        databendHelper.deleteTable(testId);
        databendHelper.createTable(createTable);
