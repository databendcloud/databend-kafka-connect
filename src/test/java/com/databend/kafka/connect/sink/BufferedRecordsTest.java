...
public class BufferedRecordsTest {
    ...
    public void testFlushExecuteFailed() throws SQLException {
        ...
        when(colDefMock.type()).thenReturn(String.valueOf(Types.VARCHAR));
        ...
        when(dbStructureMock.createOrAmendIfNecessary(Matchers.any(DatabendSinkConfig.class),
                Matchers.any(Connection.class),
                Matchers.any(TableIdentity.class),
    ...
    public void testInsertModeUpdate() throws SQLException {
        ...
        final DatabendConnection dbDialect = DatabendClient.create(config);
        assertTrue(dbDialect instanceof DatabendConnection);
        final DbStructure dbStructureMock = mock(DbStructure.class);
        when(dbStructureMock.createOrAmendIfNecessary(Matchers.any(DatabendSinkConfig.class),
                Matchers.any(Connection.class),
                Matchers.any(TableIdentity.class),
    ...
    public void testAddRecordDeleteEnabledAndNonePkMode() throws SQLException {
        props.put("delete.enabled", true);
        props.put("pk.mode", "none");
        ConfigException e = assertThrows(ConfigException.class, () -> new DatabendSinkConfig(props));
        assertEquals(
